...
// Importamos la version nativa de promesas que el driver puede usar para async/await de forma limpia
...
const server = http.createServer(async (req, res) => {
  ...
  // Cabeceras CORS manuales obligatorias para que el navegador no bloquee el Live Server
  ...
  if (req.url === '/tasks' && req.method === 'POST') {
    ...
    // Reconstruimos el flujo de datos del cuerpo (Stream chunks)
  ...
  if (req.url.startsWith('/tasks/') && req.method === 'PUT') {
    ...
    req.on('end', async () => {
      try {
        ...
        // 1. Validar si la tarea existe en la base de datos
  ...
  if (req.url.startsWith('/tasks/') && req.method === 'DELETE') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Paso B: Si pasa el filtro, ejecutamos el borrado real del ID en la BD
        ...
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ status: 'error', message: 'Fallo al eliminar en el BD: ' + error.message }));
      }
    });
    return;
  }
